...
import { isNull,isEmpty, isNil } from '../../../utils/utils';
...
@Component({
  selector: "top-holdings-investors",
  templateUrl: "./top-holdings-investors.report.html",
  styleUrls: ['./top-holdings-investors.report.scss'],
})
export class TopHoldingsInvestorsComponent
  ...
  tableData = [];
  collapsed = true;
  topholdingReportId=ReportCategory.HoldingsInvestors;
  model: any = {
    fundIds: [],
    strategyIds: [],
    regionIds: [],
    ...
    fundHoldingStatusIds: [],
    isAscending: false,
    selectedReportTypes: [this.reportType.TopHoldingInvestorInvestmentCost],
    chartMetadetaList: [],
    pCStatusIds:[],
    investorId:-1,
    sortBy: 'desc',
  ...
  loadInvestorList(){
    this.invService.getAllInvestorsList().subscribe((resp:any)=>{
      if (resp.length > 0) {
        this.investorList = resp;
        this.selectedInvestor=this.investorList[0];
        if (!isNil(this.selectedInvestorClone)) {
          this.selectedInvestor = this.selectedInvestorClone;
        }
        this.model.investorId=this.selectedInvestor.investorId;
      }
    })
  }
  ...
  getReportItems() {
    ...
    let holdingsByList = this.reportService.ReportTypeList.filter(function (
      ele: any) {
      return ele.category == ReportCategory.HoldingsInvestors;
    });
    ...
    this.reportService.getReportData(this.model).subscribe(
      (result) => {
        ...
        if (local.reportData.length > 0) {
          ...
          if (Object.values(this.reportData[0]?.FooterRow).includes("Total")) {
  ...
  apply() {
    if (!isEmpty(this.selectedInvestor))
      this.model.investorId = this.selectedInvestor.investorId;
    if (this.masterModel.strategyIds.length > 0) {
  ...
  resetForm() {
    ...
    this.model = {
      ...
      chartMetadetaList: [],
      pCStatusIds:[],
      investorId:isNil(this.selectedInvestor)??-1,
      sortBy: 'desc',
      rowCount: 20
    };
    this.masterModel = {
    ...
    this.loadInvestorList();
    if (!isNil(this.selectedInvestor)) {
      this.model.investorId=this.selectedInvestor.investorId;
    }
    this.getDealLatestQuarter();
  }
  ...
  onSelectedInvestor(item: any) {
    if (!isEmpty(item.value)) {
      this.selectedInvestorClone=item.value;
      this.selectedInvestor = item.value;
      this.apply();
    }
  }
