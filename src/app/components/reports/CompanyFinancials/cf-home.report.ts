...
@Component({
	selector: 'reports',
	templateUrl: './cf-home.report.html',
	styleUrls:["./cf-home.report.scss"],
	providers: [MessageService]
})

export class CFReportsComponent implements OnInit, OnDestroy {
    ...
    public messageService: MessageService
		...
		localStorage.setItem("headerName", "CompanyFinancials");  
	    this.currentTabId = "CompanyFinancials";
	...
	getTabList() {
		let holdingsByList = this.reportService.ReportTypeList.filter(function (
		  ...
		  holdingsByList.forEach(element=>{
			this.tabList.push({
                active: false,
                name: element.label
            });
		  });
		  this.holdingsByList =holdingsByList;
		  this.tabList[0].active =true;
		  this.kpiTabList = holdingsByList;
		  this.getCurrentTab(this.tabList[0]);
    }
	onDateSelect() {
	...
	getCurrentTab(tab: ITab)
	{
		let element = this.holdingsByList.find(x=>x.label == tab.name);
