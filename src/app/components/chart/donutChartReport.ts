...
@Component({
  selector: "app-donut-chart-report",
  template: ` <div #divDonutChart class="donutcharts" style="text-align:center"></div> `,
  styles:[`

  `]
})
export class DonutChartReportComponent
  ...
  CreateHighChart() {
    ...
    let local = this;
    let type = [this.title];
    let unit = "";//[" " + this.unit]; -Theo & Rohit does not want M
    this.color = [
      "#388E3C",
      "#00568F",
      "#FFA000",
    ...
    Highcharts.chart(this.chartContainer.nativeElement, {
      ...
      title: {
        text: `<div style='font-size:18px !important;font-weigh:bold;letter-spacing: 0.24px;color: #212121;'>${local.title}</br></br><span style="">${ Highcharts.numberFormat(local.totalValue, local.NoOfDecimals, ".", ",") + unit}</span></div>`,
        align: 'center',
  ...
  createCenter() {
    ...
    donuts
      ...
      .style("fill", "#ffff")
      .on("mouseover", function (_d: any, _i: any,event) {
        d3.select(event.target)
          .transition()
          .attr("r", local.chart_r * 0.75);
      })
      .on("mouseout", function (_d: any, _i: any, event) {
        d3.select(event.target)
          .transition()
          .duration(500)
          .ease(d3.easeBounce)
      ...
      .on("click", function (_d: any, _i: any,event) {
      ...
      .on("mouseover", function (d: any, _i: any, _j: any,event:any) {
        ...
        if (!local.fullView) {
          ...
          thisDonut.select(".value").text(function (donut_d: any) {
            return (
              local.miscService.formatNumber(d.data.val.toFixed(2)) +
              donut_d.unit
            );
          });
