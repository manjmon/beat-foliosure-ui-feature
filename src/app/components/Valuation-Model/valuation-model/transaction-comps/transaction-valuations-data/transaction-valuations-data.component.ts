...
@Component({
  selector: 'app-transaction-valuations-data',
  templateUrl: './transaction-valuations-data.component.html',
  styleUrls: ['./transaction-valuations-data.component.scss'],
})
export class TransactionValuationsDataComponent implements OnInit, OnChanges {
  ...
  filterDataBasedOnKpiName() {
    ...
    this.isWithSector = filteredKpiData[0]?.PeerSet == "" ? false : true
    let formateDataFunction = groupBy(filteredKpiData, 'PeerSet')
    let columHeader = this.ColumsWithoutSector.filter(x => x.field != "EV" && x.field != "EquityValue")
    if (this.isWithSector) {
      ...
      this.dataCollection.push(ValuationCalclation.GetCalcuationSchemaTransactionForOverAll("Mean"));
      this.dataCollection.push(ValuationCalclation.GetCalcuationSchemaTransactionForOverAll("Median"));
  ...
  prepareDataForEmpliedEv(dataSet: any, isWithSector: boolean) {
    if (isWithSector) {
      if (this.sectorList.length == 1) {
        ...
        let set = [];
        dataSet.filter(item => item.data.name === "All Comps")[0].children
          .forEach(element => {
      ...
      let set = [];
      dataSet
        .filter(data => this.termsToCalculate.includes(data.TargetName))
        .forEach(element => {
          ValuationCalclation.setImpliedERowWithoutSector(element.TargetName, element);
          set.push(element);
        });
      return set;
    }
  ...
  formatDataResponse(data: any) {
    ...
    mergeFlatArray.forEach((kpi) => {
      ...
      kpiDataArray = {
        ...
        "name": kpi.TargetName,
        "parentName": kpi.PeerSet,
        "PeerSet": kpi.PeerSet,
        "KpiId": kpi.KpiId,
        "KpiName": kpi.KpiName,
        "EquityValue": this.convertKpiValue(kpi.EquityValue, this.currencyUnit),
    ...
    this._impliedEvService.setHeaders(this.ColumsWithoutSector.filter(x => x.field !== 'EquityValue' && x.field !== 'EV'));
    this.filterDataBasedOnKpiName();
    this._impliedEvService.setdropdownvalues(this.dropDownKpi, this.selectedKPIDropdown.kpiId);
  ...
  addOverAllSectionForSector(tempData: any) {
    tempData.push(ValuationCalclation.AllCompsSectorTrans);
  }

  addOverAllSectionWithoutSector(dataCollection: any) {
    dataCollection.push(ValuationCalclation.GetCalcuationSchemaTransactionForOverAll("Mean"));
    dataCollection.push(ValuationCalclation.GetCalcuationSchemaTransactionForOverAll("Median"));
  }
  ...
  addMeanAndMedianForEachSector(sectorData: any, headers: any[]) {
    sectorData.forEach((row: any) => {
      if (!row.children.map(c => c.data.name).includes("Mean")) {
        row.children.push(ValuationCalclation.GetCalucuationSchemaTransaction("Mean"));
        row.children.push(ValuationCalclation.GetCalucuationSchemaTransaction("Median"));
      }
  ...
  calculationForWithoutSectorData(dataCollection: any, headers: any[]) {
    dataCollection
      .filter((x) => this.termsToCalculate.includes(x.TargetName))
      .forEach((row: any) => {
        headers.forEach((columnName) => {
  ...
  addMeanAndMedian(TargetName: string, columnName: string, row: any, data: any) {
    switch (TargetName) {
      // calculation of mean
      case "Mean - Overall":
  ...
  printNAForWithoutSector(rowData: any, col) {
    if (
      (rowData.TargetName === "Mean" ||
        rowData.TargetName === "Median") &&
      (col.field === "EV" || col.field === "EquityValue") 
    ){
      return "";
    }
  ...
  handleCheckBoxVal(rowData: any, event: any) {
    ...
    this._impliedEvService.setUnselectedRecords(this.unSelectedRecords);
    let headerList = this.ColumsWithoutSector.filter(x => x.field != "EV" && x.field != "EquityValue")
    if (this.isWithSector) {
